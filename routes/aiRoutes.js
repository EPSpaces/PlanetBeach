...
router.post("/ai/trash", upload.single("image"), async (req, res) => {
  try {
    ...
    let rewardMatch = response.content[0].text.match(/<reward>(\d+) tokens<\/reward>/);

    fs.unlinkSync(req.file.path);

    if (rewardMatch) {
      let rewardAmount = parseInt(rewardMatch[1]);
      console.log("Reward amount: " + rewardAmount);
      res.send({ reward: rewardAmount });
    } else {
      res.status(400).send("Error: No reward found in response");
    }
  } catch (err) {
    console.error(err);
